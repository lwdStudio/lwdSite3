...
export const ColorDripButton = ({to, className, children}) => {
    return (
        <AniLink paintDrip hex="#fcd34d" to={to} className={`text-white dark:text-white place-self-center w-48 p-3 my-3 text-center rounded-md shadow-md shadow-blue-600/50 font-medium bg-blue-600 hover:bg-yellow-300 hover:shadow-yellow-300 hover:text-black dark:hover:bg-yellow-300 dark:hover:shadow-yellow-300 dark:hover:text-black transition focus:shadow-none ${className}`}>
            {children}
        </AniLink>
    )
}
